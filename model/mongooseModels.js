...
}?ssl=true&replicaSet=${process.env.MONGO_REPLSET}`;*/
const uri = "mongodb://" + process.env.MONGO_ADDRESS + "/" + process.env.MONGO_DB;
console.log(uri);
...
const HeatMapSchema = new mongoose.Schema(
    {
        six_sig_olc: String, // Must actually be full 9 characters with two 00 padding and +
        corners: [[{ type: Number }]], // Two opposite corners of region (most likely NW, SE)
...
const MapLineSchema = new mongoose.Schema(
    {
        ...
        restrs_length: { type: Number, default: 0 },
        restrs: { type: [RestrSchema], default: [] }
    },
    {
        usePushEach: true
    }
...
const MapLineWithoutParentsSchema = new mongoose.Schema(
    {
        loc: { type: mongoose.Schema.Types.LineString, index: true },
        restrs_length: { type: Number, default: 0 },
        restrs: { type: [RestrSchema], default: [] }
    },
    {
        collection: "LinesWithoutParents",
        autoIndexId: true,
        usePushEach: true
    }
);
...
const MapLineParentSchema = new mongoose.Schema(
    {
        ...
        total_types: { type: Number, default: 0 },
        types_each: [Number], // Processed in batch on the backend, 14 or however types we end up having
        lines_length: { type: Number, default: 0 },
        lines: { type: [MapLineSchema], default: [] }
    },
    {
        collection: "MapLines",
        usePushEach: true
...
const MapLinesWithoutParents = mongoose.model("LinesWithoutParents", MapLineWithoutParentsSchema);
const HeatMaps = mongoose.model("HeatMaps", HeatMapSchema);
const MapLines = mongoose.model("Lines", MapLineSchema);
