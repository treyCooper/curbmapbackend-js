...
function addToPoints(newValue, userid, res) {
    sequelize
        .query(
            "UPDATE points set points_created=( CAST((SELECT points_created FROM points WHERE user_id=:userid) AS JSONB) || CAST(:pushedvalue AS JSONB)) WHERE user_id=:userid",
            {
                replacements: { userid: userid, pushedvalue: JSON.stringify(newValue) },
