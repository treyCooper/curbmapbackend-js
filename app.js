...
// CORS setup
var whitelist = ["http://localhost:8080", "https://curbmap.com", "https://curbmap.com:443", "http://curbmap.com:8080", "*"];
var corsOptions = {
...
// Session stuff
app.use(session({
    store: new RedisStore({host: "127.0.0.1", port: 50005, prefix: "curbmap:sessions:", client: redis, ttl: 100}),
    resave: false,
...
passport.use(new LocalStrategy(function (username, password, done) {
    findUser(username, function (nullvalue, userObject) {
        if (userObject !== false) {
            bcrypt
                .compare(password, userObject.password_hash, function (err, res) {
                    if (err) {
                        return done(err);
                    } else if (res === true) {
